...
import 'package:viser_bank/data/model/airtime/apply_topup_response_model.dart';
  ...
  submitTopUp() async {
    ...
    if(topUpResponseModel.status.toString().toLowerCase() == MyStrings.success.toLowerCase()){
      ...
      if(authorizationList.length > 1){
        Get.toNamed(RouteHelper.otpScreen,arguments:[RouteHelper.bottomNavBar,topUpResponseModel.data?.otpId,selectedAuthorizationMode?.toLowerCase().toString()]);
